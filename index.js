...
const SERVFAIL = require('native-dns-packet').consts.NAME_TO_RCODE.SERVFAIL;
...
function main() {
    ...
    server.on('request', (request, response) => {
      ...
      // TODO: Improve the endsWith logic, to not greedily match things like,  foo{baseDomain}.com.
      if(!request.question[0].name.toLowerCase().endsWith(`${args['base_domain']}`) && args.forward) {
        return forwardTo(request.question[0], args.forward).then(({rcode, answer, additional, authority}) => {
          logAnswer(answer, request)
        ...
        if (response.answer.length > 0) {
          logAnswer(response.answer, request)
        } else {
    ...
    server.on('listening', () => {
        log.info(c.gray('[*]') + ` server listening on port ${server.address().address} on ${server.address().port}`)
        if(args.forward)
          log.info(c.gray('[*]') + ` forwarding DNS questions to ${args.forward}`)
        if(args['base_domain'])
          log.info(c.gray('[*]') + ` capturing domains that end with ${args["base_domain"]}`)
    })
  ...
  server.serve(args.port, args.address)

  function logAnswer(answer, request) {
...
function parseArgs() {
    ...
    parser.addArgument(
      [ '-a', '--address' ],
      {
        help: 'What address to run the DNS server on (default: 127.0.0.1).',
    ...
    parser.addArgument(
      [ '-n', '--base-domain' ],
      {
        help: 'What domain to respond to (default: example.local).',
    ...
    parser.addArgument(
      [ '-t', '--ttl' ],
      {
        help: 'How long a TTL to give to answer that we give (default: 1)',
        defaultValue: '1',
      }
    )
    ...
    parser.addArgument(
      [ '-f', '--forward' ],
      {
        help: 'What address to forward unknown questions to (default: none, but try 8.8.8.8).',
        defaultValue: ''
      }
    )
    ...
    parser.addArgument(
      [ '-i', '--forward-filter' ],
      {
        help: 'A filter to decide which questions to forward (default: none)',
        defaultValue: ''
      }
    )
...
function forwardTo(question, forwardAddress) {
  return new Promise((resolve, reject) => {
    ...
    responseObj.rcode = SERVFAIL;

    innerRequest.on('message', function (err, innerResponse) {
      responseObj.rcode = innerResponse.header.rcode
      responseObj.answer = innerResponse.answer
      responseObj.additional = innerResponse.additional
      responseObj.authority = innerResponse.authority
    });

    innerRequest.on('error', function(err) {
      reject(err)
    })
